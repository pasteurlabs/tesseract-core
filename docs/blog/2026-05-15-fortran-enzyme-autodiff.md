...
If you work in scientific computing, you might have run into this: you have a simulation written in Fortran (or C, or C++), and now someone needs gradients — that is, derivatives of the simulation's outputs with respect to its inputs. Maybe it's for optimization, maybe inverse problems, maybe plugging the sim into an ML pipeline. Now your options are:
...
Boundary conditions are mixed — Dirichlet (hot wall at the bottom), convection/Robin (top), insulated/Neumann (sides) — and the faces use harmonic-mean conductivity, the standard trick for flux continuity across cells with different $k$.
...
Most of those steps are plumbing: lower the Fortran to IR, compile a thin C wrapper, `llvm-link` the two, run Enzyme, optimize, emit a `.so`. The two most salient ones are the LFortran IR dump and the Enzyme pass itself:

```bash
# Fortran → LLVM IR (clean enough for Enzyme to trace)
lfortran --show-llvm --no-array-bounds-checking thermal_2d.f90 > thermal_2d.ll

# ...link in the C wrapper, then synthesize the derivative
...
That U-shaped FD curve is the classic truncation-vs-roundoff trade-off. Enzyme sidesteps it by computing the analytic derivative directly, at any input, with no step size to tune.

## Putting the gradients to work
...
One `value_and_grad` call hands back the gradient with respect to all 900 unknowns at once. This is exactly the regime where reverse-mode AD (or its cousin the adjoint method) stops being a nice-to-have and becomes essential:
