...
| **Development and debugging**                  | Use {doc}`non-containerized execution </content/how-to/use>` or {doc}`tesseract-runtime serve </content/how-to/deploy>` for fast iteration, then switch to containerized HTTP for final testing.                                                                |
...
| **Tight loops on in-memory data**              | Consider {doc}`non-containerized execution </content/how-to/use>` to bypass all network/container overhead. At ~0.5ms per call, you can run thousands of iterations per second. Requires all dependencies to be available in the same local Python environment. |
