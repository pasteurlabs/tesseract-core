...
By default, Tesseracts log metrics, parameters and artifacts to a directory `mpa` in the Tesseract's `--output-path`. By setting the environment variable `MPA_DIR`, you can change that directory. (Note that, when running Tesseracts in a container, the log directory is placed inside the container.)
