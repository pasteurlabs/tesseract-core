...
The phrase "generate just" helps prevent over-engineering (like adding gradient endpoints you didn't ask for).
...
3. Finally, add gradient endpoints if needed

## Example: Generating a basic Tesseract

This example shows a minimal Tesseract without gradient endpoints. The output below was generated by Claude 3.5 Haiku.
...
**Actual code the LLM produced (Claude 3.5 Haiku):**
...
```{note}
This basic example does not implement gradient endpoints. To make a Tesseract fully differentiable, you need to:
...
## Using LLMs with gradient endpoints
...
```{seealso}
[Tesseract-JAX](https://github.com/pasteurlabs/tesseract-jax) allows you to embed Tesseracts as JAX primitives into end-to-end differentiable programs, and provides helpers for automatically generating gradient endpoints from JAX functions.
```
