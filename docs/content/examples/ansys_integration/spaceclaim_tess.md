...
## Example SpaceClaim Tesseract (`demo/showcase/spaceclaim`)

For this specific example we are looking at the SpaceX grid fin geometry shown in this [demo](https://si-tesseract.discourse.group/c/showcase/11). This specific example requires `trimesh` as a dependency. For an easy setup navigate to `demo/showcase/spaceclaim` and install the requirements in your python environment of choice
...
```{literalinclude} ../../../../demo/showcase/spaceclaim/tesseract_api.py
:language: python
...
Once the `.scscript` is ready the final step is to run SpaceClaim. Here it is easy to see how this process could be extended to any software that cannot be containorized. For example Ansys Fluent could also be wrapped in a runtime Tesseract, with the Adjoint solver used to produced gradient information, allowing the Tesseract to be differentiable.
