...
class TesseractReference:
    ...
    References are passed as ``{"type": ..., "ref": ...}``, where ``type`` is one of
    ``"api_path"``, ``"image"``, or ``"url"``, and ``ref`` locates the Tesseract
    accordingly (a path to a ``tesseract_api.py``, an image name, or a URL).
    """
