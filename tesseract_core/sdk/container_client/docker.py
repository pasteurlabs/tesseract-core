...
"""Docker (and Podman) container backend for Tesseract usage."""
...
class CLIDockerClient:
    ...
    def capabilities(self) -> Capabilities:
        """Capabilities of the Docker/Podman backend.

        Docker and Podman support the full feature set; the only difference the
        engine cares about is Podman's ``--userns keep-id`` handling, surfaced via
        the ``is_podman`` flag so call sites branch on capabilities rather than
        sniffing the backend.
        """
        if self._capabilities is None:
            ...
            self._capabilities = Capabilities(
                ...
                supports_restart_policy=True,
                supports_build=True,
                needs_user_mapping=True,
                is_podman=podman,
                name="podman" if podman else "docker",
            )
