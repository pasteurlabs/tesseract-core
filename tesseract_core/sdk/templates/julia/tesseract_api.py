...
# Pre-compute path patterns from schema at module load time
_ALL_PATTERNS = [
...
def _expand_inputs(
    ...
    # Split by differentiability, skip intermediate nodes (dicts/lists)
    ...
    for path in all_paths:
        ...
        if isinstance(value, (dict, list)):
            continue  # intermediate node, not a leaf
...
def abstract_eval(abstract_inputs):
    """Calculate output shapes from input shapes.

    Must be implemented explicitly since JAX cannot trace through Julia code.
    """
    inp = abstract_inputs.model_dump()
    return {
        "y": {"shape": inp["x"]["shape"], "dtype": inp["x"]["dtype"]},
    }
...
def jacobian_vector_product(
    ...
    # Build tangent for each diff arg: provided tangent or zero
    tangent_args = []
...
def vector_jacobian_product(
    ...
    # Map gradients back to requested paths
    result = {}
    for k, path in enumerate(diff_paths):
