...
class InputSchema(BaseModel):
    ...
    config: InputPath = Field(description="Configuration file")
...
class OutputSchema(BaseModel):
    ...
    trained_models: list[OutputPath] = Field(
        description="Pickle file containing weights of trained model"
    )
    scalers: list[OutputPath] = Field(
...
def evaluate(inputs: Any) -> Any:
    ...
    # Convert all inputs to Path objects (handles strings, InputPath, and Path)
    config = get_config()
