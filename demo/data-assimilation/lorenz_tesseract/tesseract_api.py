...
"""Lorenz 96 JAX Tesseract with a differentiable forcing parameter F.

This is the shared JAX Lorenz 96 Tesseract used by both the 4D-Var data
assimilation demo (which differentiates w.r.t. the initial state) and the
Bayesian inference demo (which differentiates w.r.t. the forcing parameter F).
Marking F as Differentiable is inert for callers that never request it in a
gradient, so a single Tesseract serves both use cases.
"""
