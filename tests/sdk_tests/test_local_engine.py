...
def test_serve_and_teardown(dummy_api_path):
    ...
    try:
        ...
        logs = local_engine.teardown(served)
...
def test_runs_in_a_different_process(dummy_tesseract_package, sample_inputs):
    ...
    with tess:
        process = tess._serve_context["served"].process
...
def test_teardown_stops_the_process(dummy_api_path):
    ...
    tess.serve()
    served = tess._serve_context["served"]
    assert served.is_running()
...
def _debug_address(served):
    ...
    match = re.search(
        r"Debugger listening on ([\d.]+):(\d+)", local_engine.logs(served)
    )
...
def test_two_tesseracts_get_distinct_debugpy_ports(dummy_api_path):
    ...
    try:
        ...
        local_engine.teardown(first)
        local_engine.teardown(second)
...
def test_inherited_runtime_override_cannot_hijack_the_debugpy_port(
    ...
    with (
        ...
        ports = {_debug_address(t._serve_context["served"])[1] for t in (first, second)}
...
def test_startup_timeout_is_reported(dummy_api_path, monkeypatch):
    ...
    def never_healthy(*args, **kwargs):
        raise local_engine.requests.exceptions.ConnectionError("nope")

    monkeypatch.setattr(local_engine.requests, "get", never_healthy)

    with pytest.raises(RuntimeError, match="did not respond to a health check"):
...
def test_teardown_escalates_to_sigkill(dummy_api_path):
    ...
    original = local_engine._TERMINATE_TIMEOUT
    local_engine._TERMINATE_TIMEOUT = monkeypatched_timeout
    try:
        os.killpg(os.getpgid(served.process.pid), signal.SIGSTOP)  # ignores SIGTERM
        local_engine.teardown(served)
    finally:
        local_engine._TERMINATE_TIMEOUT = original
